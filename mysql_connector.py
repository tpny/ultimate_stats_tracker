...
class MySQLConnector:
  def __init__(self, stats_list, user='root', host='localhost', password='lcpk1215', database='ultimate_stats'):
    # For public server
    self.user = user
  ...
  def create_tables(self):
    ...
    query = "CREATE TABLE IF NOT EXISTS PLAYERS ( id int NOT NULL AUTO_INCREMENT, player_name varchar(20) NOT NULL, team int NOT NULL, PRIMARY KEY (id), FOREIGN KEY (team) REFERENCES TEAMS(id));"
    cursor.execute(query)
    self.cnx.commit()
  ...
  def insert_or_update_play(self, team_id, game_id, plays_str, processed = False):
    cursor = self.cnx.cursor()
    query = "REPLACE INTO PLAYS (team, game, play, processed) VALUES({}, {}, '{}', {});".format(team_id, game_id, plays_str, processed)
    cursor.execute(query)
    self.cnx.commit()
    cursor.close()
  ...
  def create_game(self, home_team_id, away_team_id, game_time = None):
    cursor = self.cnx.cursor()
    if(game_time):
      query = "REPLACE INTO GAMES (home_team, away_team, game_time) VALUES({}, {}, '{}');".format(home_team_id, away_team_id, game_time)
    else:
      query = "REPLACE INTO GAMES (home_team, away_team) VALUES({}, {});".format(home_team_id, away_team_id)
    print(query)
    cursor.execute(query)
    self.cnx.commit()
    cursor.close()
  ...
  #   cursor = self.cnx.cursor()
  #   query = "DELETE FROM GAMES WHERE id = {};".format(game_id)
  #   cursor.execute(query)
  #   self.cnx.commit()
  #   cursor.close()
  ...
  def create_team(self, team_name):
    cursor = self.cnx.cursor()
    query = "INSERT INTO TEAMS (team_name) VALUES('{}');".format(team_name)
    cursor.execute(query)
    self.cnx.commit()
    cursor.close()
  ...
  def update_team(self, team_id, team_name):
    cursor = self.cnx.cursor()
    query = "UPDATE TEAMS SET team_name = '{}' WHERE id = {};".format(team_name, team_id)
    cursor.execute(query)
    self.cnx.commit()
    cursor.close()
  ...
  def create_player(self, player_name, team_id):
    cursor = self.cnx.cursor()
    query = "INSERT INTO PLAYERS (player_name, team) VALUES('{}', {});".format(player_name, team_id)
    print(query)
    cursor.execute(query)
    self.cnx.commit()
    cursor.close()

  def update_player(self, player_id, player_name, team_id):
    cursor = self.cnx.cursor()
    query = "UPDATE PLAYERS SET player_name = '{}', team = {} WHERE id = {};".format(player_name, team_id, player_id)
    cursor.execute(query)
    self.cnx.commit()
    cursor.close()
  ...
  def remove_player(self, player_id):
    cursor = self.cnx.cursor()
    query = "DELETE FROM PLAYERS WHERE id = {};".format(player_id)
    cursor.execute(query)
    query = "DELETE FROM PLAYER_STATS WHERE player = {};".format(player_id)
    cursor.execute(query)
    self.cnx.commit()
    cursor.close()
  ...
  def get_players(self, team_id = None):
    cursor = self.cnx.cursor()
    if(team_id):
      query = "SELECT * FROM PLAYERS WHERE team = {};".format(team_id)
    else:
  ...
  def get_plays(self, team_id = None, game_id = None, unprocessed = False):
    ...
    if(team_id or game_id or unprocessed):
      criti = []
      if(team_id):
        criti.append("team = {}".format(team_id))
      if(game_id):
        criti.append("game = {}".format(game_id))
      if(unprocessed):
        criti.append("processed = FALSE")
      query = "SELECT * FROM PLAYS WHERE {};".format(" AND ".join(criti))
    else:
  ...
  def get_player_stats(self, game_id = None, player_id = None, team_id = None):
    ...
    if(game_id or player_id or team_id):
      ...
      if(game_id):
        criti.append("game = {}".format(game_id))
      if(player_id):
        criti.append("player = {}".format(player_id))
      if(team_id):
        criti.append("team = {}".format(team_id))
      query = "SELECT * FROM PLAYER_STATS WHERE {};".format(" AND ".join(criti))
    else:
  ...
  def insert_or_update_player_stats(self, player_stats, players_to_id, team, game):
    ...
    for player in player_stats:
      ...
      query = "REPLACE INTO PLAYER_STATS (player, game, team, {}) VALUES({}, {}, {}, {});".format(stats_str, players_to_id[player], game, team, player_stats_str)
      cursor.execute(query)
    self.cnx.commit()
    cursor.close()